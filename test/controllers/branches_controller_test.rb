...
class BranchesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create branch" do
    assert_difference("Branch.count") do
      post branches_url, params: { branch: { company_id: @branch.company_id, name: @branch.name, user_id: @branch.user_id } }
    end
  ...
  test "should update branch" do
    patch branch_url(@branch), params: { branch: { company_id: @branch.company_id, name: @branch.name, user_id: @branch.user_id } }
